...
namespace Numbat.Commands.Modelling.NumbatHandrail
{
    internal static class HandrailGenerator
    {
        ...
        private static List<Brep> CreateSoftZigZagInfill(Curve bayCurve, double bottomZ, double topZ, double diameter, double bayLength, double tolerance)
        {
            ...
            var loopCount = Math.Max(1, (int)Math.Ceiling(length / bayLength));
            var actualBayLength = length / loopCount;
            var points = new List<Point3d>();

            points.Add(PointAtDistanceAndZ(bayCurve, 0.0, topZ));

            for (var i = 0; i < loopCount; i++)
            {
                var d0 = i * actualBayLength;
                var d1 = Math.Min(length, (i + 1) * actualBayLength);
                var q1 = d0 + (d1 - d0) * 0.22;
                var q2 = d0 + (d1 - d0) * 0.50;
                var q3 = d0 + (d1 - d0) * 0.78;

                if (i % 2 == 0)
                {
                    points.Add(PointAtDistanceAndZ(bayCurve, q1, bottomZ));
                    points.Add(PointAtDistanceAndZ(bayCurve, q2, bottomZ));
                    points.Add(PointAtDistanceAndZ(bayCurve, q3, topZ));
                    points.Add(PointAtDistanceAndZ(bayCurve, d1, topZ));
                }
                else
                {
                    points.Add(PointAtDistanceAndZ(bayCurve, q1, topZ));
                    points.Add(PointAtDistanceAndZ(bayCurve, q2, topZ));
                    points.Add(PointAtDistanceAndZ(bayCurve, q3, bottomZ));
                    points.Add(PointAtDistanceAndZ(bayCurve, d1, bottomZ));
                }
            }
            ...
            breps.AddRange(CreatePipe(curve, diameter, tolerance));
            return breps;
        }
